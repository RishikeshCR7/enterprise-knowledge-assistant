...
def setup_test_documents():
    ...
    hr_chunk = DocumentChunk(
        chunk_id="doc_hr_salary_chunk_0",
        doc_id="doc_hr_salary",
    ...
    eng_chunk = DocumentChunk(
        chunk_id="doc_eng_standards_chunk_0",
        doc_id="doc_eng_standards",
    ...
    return {"hr_id": "doc_hr_salary", "eng_id": "doc_eng_standards"}
...
def test_task_a1_rbac_cross_department_restriction(setup_test_documents):
    ...
    results = chroma_store.search("salary adjustments", k=5, user_context=eng_user)
    # Engineering user must NOT be granted access to HR Confidential salary chunk
    assert not any(r["chunk_id"] == "doc_hr_salary_chunk_0" for r in results)
    ...
    hr_results = chroma_store.search("salary adjustments", k=5, user_context=hr_user)
    assert any(r["chunk_id"] == "doc_hr_salary_chunk_0" for r in hr_results)


def test_task_a2_candidate_retrieval_functions(setup_test_documents):
    # Test retrieve_chunks
    chunks = retrieve_chunks("PEP 8 code standards", k=2)
    assert len(chunks) >= 1
    assert any("PEP 8" in c["text"] for c in chunks)

    # Test retrieve_documents
    docs = retrieve_documents("PEP 8 code standards", k=2)
    assert len(docs) >= 1
    assert "doc_id" in docs[0]
    assert "matching_chunks" in docs[0]

    # Test retrieve_with_filters
    filt_chunks = retrieve_with_filters("salary", department="HR", k=2)
